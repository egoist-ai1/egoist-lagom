...
var Zf = { "vpn-toggle": { title: `Подключение VPN`, detail: `Готовлю runtime, маршрут и системные правила.` }, "vpn-disconnect": { title: `Отключение VPN`, detail: `Останавливаю owned runtime и снимаю системные proxy/kill-switch правила.` }, "vpn-connect-node": { title: `Подключение VPN`, detail: `Подключаю выбранный сервер через owned runtime.` }, "vpn-favorite": { title: `Избранное VPN`, detail: `Сохраняю сервер в списке избранных.` }, "vpn-import": { title: `Импорт VPN`, detail: `Читаю буфер обмена и добавляю подписку или VPN-конфиг.` }, "subscription-refresh": { title: `Обновление подписки`, detail: `Запрашиваю свежий список VPN-серверов у провайдера.` }, "subscription-delete": { title: `Удаление подписки`, detail: `Удаляю подписку и её VPN-узлы из локального состояния.` }, "copy-ip": { title: `Копирование IP`, detail: `Копирую текущий внешний IP в буфер обмена.` }, speedtest: { title: `Замер скорости`, detail: `Проверяю доступность и скорость активного маршрута.` }, "internet-fix": { title: `Восстановление сети`, detail: `Отменяю только свои изменения и проверяю связность. Чужие DNS, WinHTTP/PAC и Telegram не затрагиваются.` }, "route-probe": { title: `Проверка защиты`, detail: `Проверяю смену внешнего выхода, применение маршрута в Windows и путь DNS.` }, "vpn-reapply-route": { title: `Повторное применение маршрута`, detail: `Направляю Windows на локальный прокси активной сессии.` }, "dns-apply": { title: `Настройка DNS`, detail: `Применяю DNS к активным физическим адаптерам.` }, "dns-reset": { title: `Сброс DNS`, detail: `Возвращаю DNS к настройкам Windows/DHCP.` }, "dns-check": { title: `Проверка DNS`, detail: `Проверяю DNS без требования активного VPN.` }, "dns-link-paste": { title: `Вставка DNS-ссылки`, detail: `Читаю ссылку из буфера и распознаю DoH/DoT формат.` }, "dns-link-apply": { title: `Подключение DNS-ссылки`, detail: `Проверяю DNS-ссылку и включаю защищённый System DoH канал.` }, "doh-apply": { title: `Включение System DoH`, detail: `Настраиваю штатный Windows DNS Client и проверяю системное разрешение имён.` }, "doh-reset": { title: `Отключение System DoH`, detail: `Возвращаю исходные DNS/DoH-настройки, принадлежавшие системе или пользователю.` }, "doh-restart": { title: `Проверка System DoH`, detail: `Повторно применяю DoH-политику Windows и проверяю системный DNS.` }, "gravityless-enable": { title: `Локальный DNS`, detail: `Переключаю системный DNS на локальный resolver.` }, "zapret-auto": { title: `Автоподбор обхода`, detail: `По очереди проверяю доступные профили Zapret.` }, "zapret-standalone": { title: `Запуск обхода`, detail: `Запускаю выбранный профиль в standalone-режиме.` }, "zapret-service-start": { title: `Запуск службы Zapret`, detail: `Запускаю owned службу с текущим профилем.` }, "zapret-service-install": { title: `Установка службы Zapret`, detail: `Переустанавливаю owned службу EgoistShieldZapret с выбранным профилем.` }, "zapret-service-stop": { title: `Остановка службы Zapret`, detail: `Останавливаю только owned службу EgoistShieldZapret.` }, "zapret-remove": { title: `Удаление службы Zapret`, detail: `Удаляю только owned-службу EgoistShieldZapret.` }, "zapret-reset": { title: `Сброс Zapret`, detail: `Останавливаю owned WinDriver/WinWS процессы.` }, "zapret-stop-standalone": { title: `Остановка standalone Zapret`, detail: `Останавливаю отдельный owned winws-процесс.` }, "zapret-game-filter": { title: `Игровой фильтр`, detail: `Обновляю режим GameFilter в Flowseal core.` }, "zapret-ipset-mode": { title: `IPSet Flowseal`, detail: `Переключаю режим IPSet для Discord Fix.` }, "zapret-update-ipset": { title: `Обновление IPSet`, detail: `Обновляю список IPSet из Flowseal.` }, "zapret-save-lists": { title: `Маршруты Discord`, detail: `Сохраняю пользовательские домены и IP/CIDR списки.` }, "zapret-check-updates": { title: `Обновления Flowseal`, detail: `Проверяю доступную версию Flowseal Core.` }, "zapret-install-update": { title: `Установка Flowseal Core`, detail: `Применяю совместимое обновление Flowseal Core.` }, "zapret-emergency-core": { title: `Аварийный Flowseal Core`, detail: `Переключаю ядро на fallback-версию 1.9.2.` }, "zapret-tests": { title: `Тесты Flowseal`, detail: `Открываю консоль тестов Flowseal.` }, "zapret-clean-cache": { title: `Очистка Discord-кеша`, detail: `Закрываю Discord-клиенты и очищаю только Electron cache.` }, "zapret-diagnostics": { title: `Диагностика Zapret`, detail: `Собираю состояние Flowseal, драйверов, службы и списков.` }, "tg-open": { title: `Открытие Telegram`, detail: `Передаю ссылку прокси в Telegram Desktop.` }, "tg-start": { title: `Запуск оптимизатора Telegram`, detail: `Запускаю owned proxy runtime с безопасной конфигурацией.` }, "tg-install": { title: `Установка службы оптимизатора`, detail: `Устанавливаю owned службу и автозапуск.` }, "tg-remove": { title: `Удаление службы оптимизатора`, detail: `Удаляю owned службу Telegram Proxy.` }, "tg-logs": { title: `Открытие логов оптимизатора`, detail: `Открываю папку с журналами proxy runtime.` }, "tg-update": { title: `Обновление оптимизатора Telegram`, detail: `Проверяю и при необходимости применяю совместимый headless runtime.` }, "open-log-folder": { title: `Открытие журналов`, detail: `Открываю папку журналов приложения.` }, "release-page": { title: `Открытие релиза`, detail: `Открываю страницу релиза во внешнем браузере.` }, "update-check": { title: `Проверка обновлений`, detail: `Проверяю канал обновлений и release metadata.` }, "update-auto": { title: `Настройка обновлений`, detail: `Сохраняю режим автоматической проверки.` }, "setting-autoStart": { title: `Автозапуск Windows`, detail: `Обновляю login item Windows для EgoistShield.` }, "setting-minimizeToTray": { title: `Режим трея`, detail: `Сохраняю поведение окна при закрытии приложения.` }, "setting-notifications": { title: `Уведомления`, detail: `Сохраняю показ системных уведомлений.` }, "setting-soundNotifications": { title: `Звуковые оповещения`, detail: `Меняю локальный звук для действий и ошибок.` }, "setting-autoConnect": { title: `Автоподключение VPN`, detail: `Сохраняю запуск VPN после старта приложения.` }, "setting-reconnectOnDrop": { title: `Переподключение`, detail: `Сохраняю автоповтор подключения при сбое.` }, "setting-sendSubscriptionHwid": { title: `Приватность подписок`, detail: `Обновляю согласие на передачу совместимого идентификатора провайдеру.` }, "setting-killSwitch": { title: `Kill Switch`, detail: `Сохраняю правило блокировки трафика без VPN.` }, "setting-keepLogsDays": { title: `Хранение логов`, detail: `Очищаю старые журналы по новому сроку.` }, "admin-check": { title: `Проверка прав`, detail: `Проверяю, запущено ли приложение с правами администратора.` }, "diagnostics-export": { title: `Экспорт диагностики`, detail: `Собираю архив логов, статусов runtime и настроек без секретов.` }, "vpn-auto-connect": { title: `VPN автоподключается`, detail: `Приложение запускает сохранённый маршрут.` }, "vpn-reconnect": { title: `VPN переподключается`, detail: `Пробую восстановить сессию после обрыва.` } }, Qf = `3.6.1`, $f = `10.08.2026`, ep = [[`3.6.1`, `09.09.2026`, [`Шесть экранов в новом чёрно-рубиновом интерфейсе: Обзор, VPN, DNS, Запрет, Telegram и Настройки`, `Удалены экран «Система» и управление Windows Update и Microsoft Defender`, `Чистая установка оставляет сетевые компоненты, автозапуск и автоподключение выключенными`, `Исправлены установка и восстановление сети, запуск VPN, DNS/DoH и обновление компонентов`]], [`3.5.3`, `10.08.2026`, [`Установленная фоновая служба Telegram Proxy сохраняется при обновлении и продолжает запускаться вместе с Windows до входа пользователя`, `Setup переносит только подтверждённый Automatic/LocalSystem intent, обновляет runtime и wrapper из нового пакета и проверяет SCM, recovery и локальный порт`, `Чистая установка остаётся opt-in: Telegram Proxy не включается без явного нажатия «Установить фоновую службу»`]], [`3.5.2`, `09.08.2026`, [`DNS/DoH теперь честно показывает автономную работу: системные настройки Windows продолжают действовать после закрытия Egoist Shield, а локальный резолвер — только при работающей службе`, `Параллельная проверка маршрута больше не записывает штатно отменённые fallback-запросы как сетевые ошибки`, `Сетевые функции, службы, интерфейс и установщик повторно проходят полный типизированный, поведенческий, стрессовый и пакетный контроль`]], [`3.5.1`, `09.08.2026`, [`Исправлена комплектация доверенного updater: корневой ключ и подписанный registry теперь входят в установленное приложение и проверяются при упаковке`, `Уведомления об успехе, предупреждении и ошибке автоматически исчезают после чтения и всегда доступны для ручного закрытия`, `Публичный stable-канал проверяется теми же Ed25519 и hash-инвариантами из фактически упакованной версии`]], [`3.5.0`, `09.08.2026`, [`Одна кнопка проверяет подписанный stable-канал, загружает, проверяет и запускает обновление без повторного запроса внутри приложения`, `Setup подтверждается Ed25519, SHA-256, SHA-512, размером и GitHub asset digest; 404 и сетевые ошибки больше не считаются актуальной версией`, `Прерванная загрузка безопасно продолжается, а фирменный мастер перезапускает установленную или восстановленную версию`]], [`3.4.3`, `09.08.2026`, [`Core заранее создаёт защищённый журнал системной операции и безопасно читает его одновременно с PowerShell worker-ом`, `Если финальное событие потеряно, Windows Update и Defender подтверждаются независимым readback без ложного успеха`, `Ошибка результата больше не оставляет пользователя с техническим сообщением без понятного состояния`]], [`3.4.2`, `09.08.2026`, [`Windows Update и Microsoft Defender показывают однозначный проверенный итог каждой операции`, `Core повторно читает финальное событие системного worker-а и больше не теряет успешный результат при exit code 0`, `Состояние автообновлений определяется отдельно от online search, поэтому действие всегда соответствует реальной политике`, `Раздел «Запрет / обход» переименован в компактное «Запрет»`]], [`3.4.1`, `09.08.2026`, [`Экран «Система» сокращён до двух проверяемых функций: Windows Update и Microsoft Defender`, `Исправлен запуск долговечных Core-задач: настройка кодировки больше не конфликтует с отключённым перенаправлением stdout`, `Максимальный профиль Defender использует только доступные Set-MpPreference-параметры и запускается после подтверждения Tamper Protection`, `Обновление с 3.4.0 безопасно снимает только доказанно принадлежащие удалённым модулям настройки`]], [`3.4.0`, `08.08.2026`, [`Новый System Center показывает фактическое состояние System Restore, Windows Update, Defender, приватности, очистки и целостности Windows`, `Системные изменения выполняются типизированными рецептами Core-службы с планом, readback, журналом и безопасным возобновлением`, `Defender и Windows Update больше не отключаются через неподдерживаемые ACL-служб; MDM, GPO, Tamper Protection и сторонний антивирус отображаются честно`]], [`3.3.7`, `08.08.2026`, [`Повторный запуск после прерванного обновления восстанавливает доступ только к проверенному quarantine-каталогу Egoist Shield и завершает установку`, `Старый SYSTEM-only ACL на optional runtime больше не блокирует удаление прошлой версии`, `Восстановление отклоняет junction и symlink, поэтому исправление прав не может выйти за границы файлов Egoist Shield`]], [`3.3.6`, `08.08.2026`, [`Статусы профилей Zapret размещены в отдельной компактной строке и больше не перекрывают список конфигов`, `Персональные клиентские Windows 10/11 определяются по реальным возможностям системы, а не по устаревающему списку номеров сборок`, `MDM и Microsoft Defender for Endpoint считаются активными только при подтверждённой регистрации или онбординге, а не по оставшимся ключам реестра`, `Экран «Система» показывает точный шаг включения защиты системы и открывает действия после создания проверяемой точки восстановления`]], [`3.3.5`, `03.08.2026`, [`Повторный запуск установщика автоматически завершает или очищает хвосты прерванного обновления вместо ошибки registration backup already exists`, `Путь собственной Core-службы в Program Files больше не обрезается до C:\\Program, даже если старый бинарник временно находится в карантине`, `Мастер прекращает установку до очистки файлов, если Recover действительно не смог безопасно завершить прошлую транзакцию`, `Пустые runtime-манифесты прежних сборок распознаются как отсутствие компонентов и больше не блокируют обновление`]], [`3.3.4`, `03.08.2026`, [`Flowseal 1.10.0 встроен полностью и проверяется по официальному SHA-256 даже при исчерпанном лимите GitHub API`, `Автоподбор Zapret проверяет все профили, сравнивает полноту доступа и задержку и только затем выбирает лучший`, `Официальные имена профилей с круглыми скобками принимаются Core-службой без ослабления защиты от командных метасимволов`, `Экран «Система» принимает потоковый итог диагностики, поэтому статус и действия больше не блокируются при exit code 0`, `Восстановление интернета и проверка защиты используют системный DNS Windows и корректно работают с нативным DoH`]], [`3.3.3`, `02.08.2026`, [`Установщик определяет реальные процессы, удерживающие прошлую версию, повторяет атомарную замену и никогда не завершает сторонние приложения`, `Старая служба из AppData безопасно снимается перед миграцией в защищённый ProgramData, поэтому DNS и фоновые компоненты больше не упираются в запрет доверенного пути`, `Перед завершением установки проверяются Core, DNS, Telegram Proxy, Xray, sing-box, WinDivert, Zapret-профили и SHA-256 каждого файла из runtime-манифеста`, `Сетевые функции после чистой установки остаются выключенными до выбора пользователя; встроенные компоненты и профили уже готовы к запуску`]], [`3.3.2`, `02.08.2026`, [`Новый экран «Система» показывает реальное состояние Windows Update и Microsoft Defender, ход операций, отклонения и доступный откат`, `Все изменения System Control создают точный снимок, журнал незавершённой операции и восстанавливают только принадлежащие Egoist Shield значения`, `Полный режим Defender использует Core-службу в Safe Mode и аварийно возвращает обычную загрузку при сбое`, `Обновление поверх прошлой версии ремонтирует ACL журнала установщика и не блокируется отказом старого Core-бинарника`]], [`3.3.1`, `01.08.2026`, [`Обновление Flowseal стало одним действием: приложение проверяет релиз и сразу ставит его, а при недоступном GitHub называет причину вместо «подробности в логах»`, `Проверка доступа показывает OpenAI, Claude и Gemini одним рядом с реальной задержкой: доступом считается только установленное соединение, а не ответ резолвера`, `Отчёт о защите больше не противоречит сам себе: заголовок называет предмет проверки, тост повторяет её вердикт, а каждая кнопка действительно работает`, `Журнал Telegram Proxy отсортирован по времени, показывает дату у записей прошлых суток и усекает длинные строки многоточием`, `Остановленный обход не показывает нулевой таймер, удаление собственных служб выглядит и подтверждается как удаление, а кольцо фокуса больше не читается как ошибка`]], [`3.3.0`, `31.07.2026`, [`Фоновая служба EgoistShieldCore транзакционно управляет DNS и жизненным циклом собственных Windows-служб с восстановлением после сбоя`, `System DoH, Gravityless DNS, Zapret и Telegram Proxy используют защищённый runtime в ProgramData и не принимают произвольные пути к исполняемым файлам`, `Telegram Proxy получил строгую проверку конфигурации, быстрый пул по умолчанию и четыре понятных действия без потери расширенных настроек`, `Проверка VPN-маршрута завершается по первому успешному endpoint, а пинг показывает медиану до выбранного сервера без переноса старого результата`, `Счётчик VPN-трафика больше не выдаёт общий трафик компьютера за трафик туннеля`, `Core устойчив к медленным локальным pipe-клиентам и не зависит от языка вывода Windows при проверке фоновых служб`]], [`3.2.1`, `30.07.2026`, [`Установщик больше не трогает чужие VPN, DNS-резолверы и драйверы WinDivert: любое действие требует доказательства пути`, `Неудачное обновление автоматически возвращает прошлую рабочую версию, а прерванное — доводится до конца при следующем запуске`, `Системный прокси и DNS изменяются транзакционно: исходные настройки восстанавливаются после сбоя, падения и удаления`, `«Проверить защиту» даёт честный вердикт вместо инвертированного флага: внешний DNS больше не считается утечкой`, `«Восстановить интернет» отменяет только свои изменения, сохраняет корпоративный прокси и подтверждает связность на самом деле`, `Меньше фоновой нагрузки: нет повторяющихся промахов чтения DNS, спокойнее опрос, журнал по умолчанию info`, `Настройки и узлы не теряются при ошибке записи, а несохранённая конфигурация Telegram не затирается фоновым опросом`, `Замер скорости честно назван, показывает объём и длительность и действительно отменяется`]], [`3.2.0`, `29.07.2026`, [`Мгновенный отклик действий без искусственной задержки и полного перезапроса состояния`, `VPN, DNS, Zapret и Telegram Proxy координируют конфликтующие системные изменения`, `Компактные настройки, нативные подсказки и реальная проверка upstream-маршрута Telegram`]], [`3.1.1`, `29.07.2026`, [`Telegram Proxy сразу использует рабочий Cloudflare fallback на сетях, где прямые Telegram WebSocket-адреса заблокированы`, `Старый DC2/DC4 маршрут автоматически мигрируется без смены secret или локального порта`, `Пустой список прямых DC теперь является явным и проверяемым fallback-only режимом`]], [`3.1.0`, `27.07.2026`, [`Окно стало просторнее, а настройки — компактнее: подробности открываются по наведению и с клавиатуры`, `Автопереподключение перенесено в основной процесс и работает в трее с экспоненциальной задержкой`, `Ручное отключение гарантированно отменяет ожидающие и устаревшие попытки восстановления`, `Анимации переходов стали короче и спокойнее`]], [`3.0.0`, `26.07.2026`, [`Реальная проверка типов, устранены 66 скрытых ошибок и восстановлен строгий контракт IPC`, `Системный прокси и временные конфигурации восстанавливаются после аварийного завершения`, `Статистика трафика читает фактический порт сессии, секреты редактируются в журналах, внешние GeoIP-запросы отключены по умолчанию`, `Поведенческие тесты сетевого ядра, доступный интерфейс и фирменный установщик`]], [`2.1.3`, `16.07.2026`, [`Флаги и страны корректно восстанавливаются из повреждённых названий подписки`, `Сохранённые названия узлов и провайдера исправляются без изменения URI и ключей`, `Замер скорости использует адаптивные выборки, медиану, jitter, loaded latency, потери HTTPS-проб и оценку достоверности`, `DNS и фоновые службы восстанавливаются без дублирующих процессов и legacy-задач автозапуска`]], [`2.1.2`, `16.07.2026`, [`Окно запоминает размер, положение и maximized-состояние`, `Первый запуск использует всю рабочую область экрана`, `Сохранённый browser zoom больше не уменьшает интерфейс`, `Повреждённые UTF-8 названия VPN-узлов восстанавливаются автоматически`]], [`2.1.1`, `16.07.2026`, [`VPN считается подключённым только после проверки внешнего маршрута`, `Неподписанные обновления больше не устанавливаются автоматически`, `HWID подписки выключен до явного согласия`, `Компактная кнопка подключения и улучшенная доступность`]], [`2.1.0`, `10.07.2026`, [`Новый command center и переработанная кнопка подключения`, `Graceful shutdown ждёт очистку owned runtime`, `Legacy Kill Switch отключён до безопасной WFP-службы`, `Анимация учитывает системную настройку reduced motion`]], [`1.0.0`, `23.05.2026`, [`Новая оболочка на Electron/React`, `Адаптированы настройки EgoistShield 5.4.0`, `Добавлены живые статусы и явные процессы действий`]], [`5.4.0 baseline`, `10.05.2026`, [`Gravityless DNS: 127.0.0.1:53 -> dns.gravityless.space:8443`, `Flowseal Zapret 1.9.9a и профили general/ALT/FAKE TLS`, `TG Proxy 1.7.0: 127.0.0.1:1443, DC IP, буферы и обновления`]]];
function tp(e2) {
  try {
...
function np() {
  ...
  let S2 = O.useCallback(async (e3, t3, n3) => {
    ...
    try {
      ...
      if (o3()) {
        ...
        s2({ id: e3, tone: t4, title: l3 ? `Автоподбор остановлен` : a4?.title ?? c3.title, detail: l3 ? `В истории оставлены только полностью проверенные профили` : a4?.detail ?? n3, startedAt: Date.now() }), p2.current && !l3 && tp(t4 === `bad` ? `bad` : `good`);
      }
  ...
  O.useEffect(() => {
    let e3 = window.egoistAPI?.autoConnect?.onAutoConnect?.(() => {
      S2(`vpn-auto-connect`, () => Z(`vpn.connect`, window.egoistAPI?.vpn?.connect), `VPN автоподключение запущено`);
    });
    return () => e3?.();
  }, [S2]), O.useEffect(() => {
    ...
    Z(`updater.getLastResult`, window.egoistAPI?.updater?.getLastResult).then((e3) => {
      e3 && s2({ id: `update-completed`, tone: e3.ok === true ? `good` : `bad`, title: e3.ok === true ? `Egoist Shield обновлён` : `Обновление не завершено`, detail: Q(e3.message) ?? (e3.ok === true ? `Новая версия установлена и запущена.` : `Предыдущая версия восстановлена.`), startedAt: Date.now() });
    }).catch(() => void 0);
  }, []), O.useEffect(() => {
    ...
    s2({ id: `vpn-reconnect`, tone: `warn`, title: e3.phase === `reconnecting` ? `VPN переподключается` : `Соединение потеряно`, detail: e3.phase === `reconnecting` ? `Попытка ${Math.max(1, Number(e3.attempt) || 1)}. Ручное отключение отменит автоповтор.` : `Следующая попытка через ${r3 ?? `несколько`} с. ${Q(e3.lastReason) ?? `Маршрут недоступен.`}`, startedAt: Date.now() });
...
Egoist Shield подтверждает внешний VPN-маршрут до показа защищённого состояния, изменяет системные настройки транзакционно и управляет только собственными процессами и службами.`, tone: `info` });
    })();
...
function sp({ activeScreen: e2, onAppInfo: t2, onNavigate: n2, snapshot: r2 }) {
  return (0, V.jsxs)(`aside`, { className: `sidebar`, "aria-label": `Навигация Egoist Shield`, children: [(0, V.jsxs)(`div`, { className: `brand`, children: [(0, V.jsx)(`div`, { className: `brand-mark`, "aria-hidden": `true`, children: (0, V.jsx)(`img`, { className: `brand-logo`, src: Yd, alt: `` }) }), (0, V.jsx)(`strong`, { children: `Egoist Shield` })] }), (0, V.jsx)(`nav`, { className: `nav-list`, children: Cf.map((t3) => {
...
function lp({ confirmAction: e2, onCloseSpeedPanel: t2, onNavigate: n2, runAction: r2, snapshot: i2, speedPanelVisible: a2 }) {
  ...
  return (0, V.jsxs)(`div`, { className: `dashboard-layout dashboard-v21`, children: [(0, V.jsxs)(Mp, { className: `identity-panel dashboard-identity`, children: [(0, V.jsxs)(`div`, { className: `identity-heading`, children: [(0, V.jsx)(`span`, { className: `status-pill ${f2}`, children: d2 }), (0, V.jsx)(`button`, { className: `mini-link`, type: `button`, onClick: () => n2(`vpn`), children: `Выбрать сервер` })] }), (0, V.jsxs)(`button`, { className: `identity-card copyable`, disabled: !h2, type: `button`, onClick: () => {
    ...
    if (c2) {
      e2(`Отключить VPN`, `Приложение остановит owned runtime и снимет системные proxy/kill-switch правила.`, `Отключить`, () => r2(`vpn-disconnect`, () => Z(`vpn.disconnect`, window.egoistAPI?.vpn?.disconnect), `VPN отключен`));
      return;
    }
    r2(`vpn-toggle`, () => Z(`vpn.connect`, window.egoistAPI?.vpn?.connect), `VPN подключается`);
  } }), (0, V.jsxs)(`div`, { className: `power-caption`, children: [(0, V.jsx)(`span`, { children: `VPN` }), (0, V.jsx)(`strong`, { children: i2.busy === `vpn-disconnect` ? `ОТКЛЮЧЕНИЕ...` : i2.busy === `vpn-toggle` || l2 ? `ПРОВЕРКА...` : c2 ? `ПОДКЛЮЧЕНО` : u2 ? `ОШИБКА` : `ОТКЛЮЧЕНО` }), (0, V.jsx)(`em`, { children: c2 ? b2 : `00:00:00` })] })] }), (0, V.jsxs)(Mp, { className: `status-rail dashboard-status`, children: [(0, V.jsx)(Pp, { label: `VPN`, value: d2, tone: f2 }), (0, V.jsx)(Pp, { label: `DNS`, value: Pm(i2.dns, i2.systemDoh), tone: Wm(i2.dns, i2.systemDoh) }), (0, V.jsx)(Pp, { label: `Запрет`, value: p2 ? `Работает` : `Остановлен`, tone: p2 ? `good` : `idle` }), (0, V.jsx)(Pp, { label: `Telegram`, value: i2.telegram?.running ? `Оптимизируется` : `Не запущен`, tone: i2.telegram?.running ? `good` : `idle` }), (0, V.jsxs)(`div`, { className: `rail-metrics`, children: [(0, V.jsx)(`span`, { title: `Медиана трёх TCP-подключений до выбранного VPN-сервера; это не ICMP и не задержка до всех сайтов`, children: `До сервера` }), (0, V.jsx)(`strong`, { children: c2 ? v2 == null ? `проверяется` : (0, V.jsxs)(V.Fragment, { children: [(0, V.jsx)(Ap, { format: (e3) => String(Math.round(e3)), value: v2 }), ` мс`] }) : `VPN не подключен` }), (0, V.jsx)(Rp, { active: c2 && v2 != null, series: i2.trafficSeries })] }), (0, V.jsxs)(`div`, { className: `rail-traffic`, children: [(0, V.jsx)(`span`, { children: `VPN трафик` }), (0, V.jsx)(`strong`, { children: c2 ? y2 ? (0, V.jsxs)(V.Fragment, { children: [`↓ `, (0, V.jsx)(Ap, { format: Gm, value: i2.traffic.rx }), ` ↑ `, (0, V.jsx)(Ap, { format: Gm, value: i2.traffic.tx })] }) : `ожидание данных` : `VPN не подключен` })] })] }), (0, V.jsx)(`div`, { className: `dashboard-bottom dashboard-operations`, children: (0, V.jsxs)(`div`, { className: `quick-actions`, children: [(0, V.jsx)(zp, { busy: i2.busy === `speedtest`, icon: _d, progress: i2.busy === `speedtest` ? te2 : null, result: !!i2.speedtest, title: `Замер скорости`, text: ie2, onClick: () => {
    r2(`speedtest`, () => Z(`system.speedtest`, window.egoistAPI?.system?.speedtest), `Замер скорости выполнен`);
  } }), (0, V.jsx)(zp, { busy: i2.busy === `internet-fix`, icon: Fd, title: `Восстановить интернет`, text: i2.busy === `internet-fix` ? `Восстанавливаю и проверяю...` : `Вернуть только то, что менял Egoist Shield`, onClick: () => e2(`Восстановить интернет`, `Отменяется только то, что изменял Egoist Shield.

Будет сделано
• остановлен собственный VPN runtime;
• восстановлен исходный системный прокси;
• удалены собственные правила Kill Switch;
• очищен кэш DNS;
• Discord-обход вернётся в состояние до подключения VPN.
...
var dp = { protected: { title: `Соединение защищено`, lead: `Трафик идёт через VPN, и Windows действительно направлен на него.`, tone: `good` }, partial: { title: `Защита работает частично`, lead: `Часть проверок не подтвердилась. Ниже отмечено, что именно и что можно сделать.`, tone: `warn` }, unprotected: { title: `Защита не работает`, lead: `VPN включён, но трафик защищён не полностью. Ниже — что именно не сработало.`, tone: `bad` }, inconclusive: { title: `Проверить не удалось`, lead: `Сеть отвечала нестабильно, поэтому результат не подтверждён ни в одну сторону. Повторите проверку.`, tone: `info` }, not_applicable: { title: `VPN не подключён`, lead: `Проверять защиту пока нечего. Подключите VPN на экране «VPN» — после этого проверка покажет, действительно ли трафик идёт через него.`, tone: `info` } }, fp = [`восстановить интернет`, `повторить проверку`, `применить маршрут заново`, `переподключить vpn`, `выбрать другой сервер`, `открыть настройки dns`, `применить настройки dns заново`];
function pp(e2) {
...
function Sp({ runAction: e2, snapshot: t2 }) {
  ...
  let v2 = (e3) => {
    ...
    if (r3 <= 0 && i3 <= 0) {
      ...
      throw Error(Q(e4) ?? `Не найдено поддерживаемых VPN-серверов или подписок.`);
    }
    return n3;
  }, `VPN-импорт выполнен`), [e2]), x2 = O.useCallback(() => e2(`subscription-refresh`, async () => {
  ...
  }) : (0, V.jsxs)(`div`, { className: `vpn-empty`, children: [(0, V.jsx)(Ad, { size: 28 }), (0, V.jsx)(`strong`, { children: `Нет серверов VPN` }), (0, V.jsx)(`span`, { children: `Импортируй подписку или конфиг, и здесь появятся реальные узлы с live ping.` })] }) })] }), (0, V.jsxs)(`div`, { className: `vpn-side-stack`, children: [(0, V.jsxs)(Mp, { className: `vpn-subscription-panel`, children: [(0, V.jsx)(Np, { children: `Подписка` }), (0, V.jsxs)(`div`, { className: `subscription-status`, children: [(0, V.jsx)(Fp, { tone: g2 ? `good` : `warn` }), (0, V.jsx)(`strong`, { children: g2 ? `Подписка активна` : d2 ? `Нет серверов` : `Требует импорта` })] }), (0, V.jsxs)(`div`, { className: `kv-list`, children: [(0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Провайдер` }), (0, V.jsx)(`strong`, { children: f2 })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Заканчивается` }), (0, V.jsx)(`strong`, { children: p2 })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Трафик` }), (0, V.jsx)(`strong`, { children: m2 })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Последнее обновление` }), (0, V.jsx)(`strong`, { children: Fm(d2?.lastUpdated) })] })] }), (0, V.jsx)(`button`, { className: `btn-secondary full`, type: `button`, onClick: () => void x2(), children: `Обновить подписку` }), (0, V.jsx)(`button`, { className: `btn-danger full`, disabled: !d2, type: `button`, onClick: () => void S2(), children: `Удалить подписку` })] }), (0, V.jsxs)(Mp, { className: `vpn-provider-panel`, children: [(0, V.jsx)(Np, { children: `Активный маршрут` }), (0, V.jsxs)(`div`, { className: `kv-list`, children: [(0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Сервер` }), (0, V.jsx)(`strong`, { children: dm(t2.vpn, t2.state) })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Провайдер` }), (0, V.jsx)(`strong`, { children: f2 })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Страна` }), (0, V.jsx)(`strong`, { children: u2 ? `${u2.country}${u2.city ? `, ${u2.city}` : ``}` : `Узел не выбран` })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Runtime` }), (0, V.jsx)(`strong`, { children: Q(t2.vpn?.runtimeKind) ?? `не запущен` })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Состояние` }), (0, V.jsx)(`strong`, { children: wm(t2.vpn) ? `Маршрут проверен` : t2.vpn?.connected ? `Проверяется` : `Отключён` })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Внешний IP` }), (0, V.jsx)(`strong`, { children: wm(t2.vpn) ? Q(t2.vpn?.egressIp) ?? `подтверждён` : `не подтверждён` })] }), (0, V.jsxs)(`div`, { children: [(0, V.jsx)(`span`, { children: `Пинг` }), (0, V.jsx)(`strong`, { children: wm(t2.vpn) ? `${$(t2.vpn?.pingMs, t2.health?.pingMs) ?? `...`} мс` : `VPN не подключен` })] })] })] })] })] });
}
function Cp(e2) {
...
function Tp({ activity: e2, confirmAction: t2, runAction: n2, snapshot: r2 }) {
  ...
  return (0, V.jsxs)(`div`, { className: `zapret-layout`, children: [(0, V.jsxs)(Mp, { className: `config-list`, children: [(0, V.jsx)(Np, { children: `Конфиги обхода` }), (0, V.jsxs)(`div`, { "aria-label": `Статусы проверки профилей`, className: `config-legend`, role: `group`, children: [(0, V.jsxs)(`span`, { children: [(0, V.jsx)(Fp, { tone: `good` }), `Успешно`] }), (0, V.jsxs)(`span`, { children: [(0, V.jsx)(Fp, { tone: `bad` }), `Ошибка`] }), (0, V.jsxs)(`span`, { children: [(0, V.jsx)(Fp, { tone: `idle` }), `Не проверен`] })] }), (0, V.jsx)(`div`, { className: `config-scroll`, children: fe2 ? c2.map((e3) => (0, V.jsxs)(`button`, { className: E2 === e3.name ? `config-row active` : `config-row`, onClick: () => a2(e3.name), type: `button`, children: [(0, V.jsxs)(`span`, { children: [(0, V.jsx)(`strong`, { children: e3.name }), (0, V.jsx)(`em`, { children: e3.detail })] }), (0, V.jsxs)(`span`, { className: `config-row-status`, title: Ip(e3.tone), children: [(0, V.jsx)(Fp, { tone: e3.tone }), (0, V.jsx)(`span`, { className: `sr-only`, children: Ip(e3.tone) })] })] }, e3.name)) : (0, V.jsxs)(`div`, { className: `empty-table-state compact-empty`, children: [(0, V.jsx)(bd, { size: 17 }), (0, V.jsx)(`span`, { children: `Профили обхода ещё не загружены` })] }) }), (0, V.jsxs)(`div`, { className: `config-actions`, children: [(0, V.jsx)(`button`, { className: `btn-primary full`, "data-testid": `zapret-auto`, type: `button`, onClick: () => {
  ...
  }, children: ee2 ? `Останавливаю…` : `Отменить автоподбор` }) : null, (0, V.jsx)(`button`, { className: `btn-secondary full`, "data-testid": `zapret-start`, disabled: !E2, type: `button`, onClick: () => t2(`Запустить обход`, `Обход запустится как обычный процесс приложения и остановится вместе с ним. Для постоянной работы используйте «Запустить службу».`, `Запустить`, () => n2(`zapret-standalone`, () => Z(`zapret.startStandalone`, window.egoistAPI?.zapret?.startStandalone, E2), `Zapret запущен`)), children: `Запустить` }), (0, V.jsx)(`button`, { className: `btn-secondary full`, "data-testid": `zapret-service-start`, disabled: !E2, type: `button`, onClick: () => t2(`Запустить службу Zapret`, `Профиль будет закреплён за собственной службой EgoistShieldZapret: обход продолжит работать в фоне и переживёт перезагрузку.`, `Запустить`, () => n2(`zapret-service-start`, async () => {
  ...
  }, `Служба Zapret запущена`)), children: `Служба` }), (0, V.jsx)(`button`, { className: `btn-danger full`, "data-testid": `zapret-remove`, type: `button`, onClick: () => t2(`Удалить службу Zapret`, `Будет удалена только собственная служба EgoistShieldZapret. Обход перестанет запускаться автоматически после перезагрузки.

Чужие службы и драйверы не затрагиваются.`, `Удалить`, () => n2(`zapret-remove`, () => Z(`zapret.removeService`, window.egoistAPI?.zapret?.removeService), `Служба Zapret удалена`)), children: `Удалить службу` }), (0, V.jsx)(`button`, { className: `btn-secondary full`, "data-testid": `zapret-clean-cache`, type: `button`, onClick: () => t2(`Очистить кэш всех Discord-клиентов`, `Будут закрыты Discord, Discord PTB, Discord Canary и Vesktop, затем очищен только Electron cache. Учётки и Local Storage не трогаются.`, `Очистить`, () => n2(`zapret-clean-cache`, () => Z(`zapret.cleanDiscordCache`, window.egoistAPI?.zapret?.cleanDiscordCache, `all`), `Кеш всех Discord-клиентов очищен`)), children: `Кэш Discord` }), (0, V.jsx)(`button`, { className: `btn-danger full`, "data-testid": `zapret-reset`, type: `button`, onClick: () => t2(`Сбросить WinDriver / WinWS`, `EgoistShield остановит только собственные процессы и службы Zapret и переустановит драйвер WinDivert. Чужие обходы и драйверы не затрагиваются.`, `Сбросить`, () => n2(`zapret-reset`, () => Z(`zapret.resetNetworkState`, window.egoistAPI?.zapret?.resetNetworkState), `Zapret сброшен`)), children: `Сброс WinWS` })] })] }), (0, V.jsxs)(`div`, { className: `zapret-main`, children: [(0, V.jsxs)(Mp, { className: `zapret-status-panel`, children: [(0, V.jsx)(Np, { children: `Текущее состояние` }), (0, V.jsx)(Wp, { rows: [[`Активный конфиг`, he2 || `Профиль не выбран`], [`Статус`, ge2], [`Время работы`, _e2]] }), le2 && !ue2 && !T2 ? (0, V.jsxs)(`div`, { className: `zapret-best-banner`, "data-testid": `zapret-best-banner`, children: [(0, V.jsxs)(`div`, { children: [(0, V.jsx)(`strong`, { children: `Автоподбор завершён: ${le2}` }), (0, V.jsx)(`small`, { children: `Остальные удачные профили доступны в истории автоподбора.` })] }), (0, V.jsx)(`button`, { className: `btn-primary`, "data-testid": `zapret-best-connect`, type: `button`, onClick: () => void n2(`zapret-standalone`, () => Z(`zapret.startStandalone`, window.egoistAPI?.zapret?.startStandalone, le2), `Zapret запущен`), children: `Подключить` })] }) : null] }), (0, V.jsxs)(Mp, { className: `zapret-workbench`, children: [(0, V.jsx)(`div`, { "aria-label": `Разделы обхода`, className: `tab-strip`, role: `tablist`, children: [[`history`, `История`], [`routes`, `Маршруты`]].map(([e3, t3]) => (0, V.jsx)(`button`, { "aria-selected": u2 === e3, className: u2 === e3 ? `active` : ``, role: `tab`, onClick: () => d2(e3), type: `button`, children: t3 }, e3)) }), u2 === `history` && (0, V.jsxs)(`div`, { className: `zapret-history-panel`, children: [(0, V.jsx)(Np, { children: `История тестов` }), r2.zapretAutoSelect ? (0, V.jsxs)(`div`, { className: `test-table`, children: [(0, V.jsxs)(`div`, { className: `table-head`, children: [(0, V.jsx)(`span`, { children: `Конфиг` }), (0, V.jsx)(`span`, { children: `Результат` }), (0, V.jsx)(`span`, { children: `Пинг` }), (0, V.jsx)(`span`, { children: `Успех` })] }), ve2.map((e3) => {
    let t3 = f2 === e3.name;
...
function cm(e2, t2) {
  ...
  if (t2.startsWith(`dns`) || t2.includes(`doh`) || t2.includes(`gravityless`)) {
    let e3 = qd(n2);
    return e3 === `core` ? lm(`DNS-CORE`, `Служба DNS не готова`, `Системная служба Egoist Shield не ответила. Подождите несколько секунд и повторите; если ошибка останется — переустановите текущую версию.`) : e3 === `admin` ? lm(`DNS-ADMIN`, `Ошибка DNS`, `Нужны права администратора для изменения DNS Windows.`) : e3 === `no-interface` ? lm(`DNS-NO-IFACE`, `Ошибка DNS`, `Активный сетевой адаптер Windows не найден.`) : e3 === `verify` ? lm(`DNS-VERIFY`, `Ошибка DNS`, `Windows не подтвердила применение новых DNS.`) : lm(`DNS-APPLY`, `Ошибка DNS`, `DNS не применён. Проверьте права администратора и активный адаптер.`);
  }
  return t2.startsWith(`zapret`) ? /getaddrinfo|enotfound|econnrefused|econnreset|network|сет[ьи] недоступ|fetch failed|http 5\d\d/i.test(n2) ? lm(`ZAPRET-NETWORK`, `Ошибка Zapret`, `GitHub недоступен: проверьте интернет и повторите. Локальный Core продолжает работать.`) : /http 403|rate limit|превышен лимит/i.test(n2) ? lm(`ZAPRET-RATE-LIMIT`, `Ошибка Zapret`, `GitHub временно ограничил запросы. Повторите через несколько минут.`) : /conflict|конфликт|goodbyedpi|external-zapret|zapret_discord/i.test(n2) ? lm(`ZAPRET-CONFLICT`, `Ошибка Zapret`, `Найден конфликтующий обход или старая служба.`) : /windivert|driver|драйвер/i.test(n2) ? lm(`ZAPRET-DRIVER`, `Ошибка Zapret`, `Драйвер WinDivert не готов или занят другим процессом.`) : /timeout|timed out|таймаут|не ответил вовремя/i.test(n2) ? lm(`ZAPRET-TIMEOUT`, `Ошибка Zapret`, `Профиль не ответил за отведённое время.`) : /not found|не найден|missing/i.test(n2) ? lm(`ZAPRET-RUNTIME`, `Ошибка Zapret`, `Не найден runtime-файл Zapret.`) : lm(`ZAPRET-ACTION`, `Ошибка Zapret`, n2 || `Действие Zapret не выполнено. Подробности сохранены в логах.`) : r2.includes(`command failed`) || r2.includes(`powershell`) || r2.includes(`cmd.exe`) ? lm(`SYSTEM-COMMAND`, `Системная ошибка`, `Системная команда завершилась с ошибкой. Подробности сохранены в логах.`) : lm(`APP-ACTION`, `Действие не выполнено`, n2 || `Операция завершилась ошибкой.`);
}
...
function Tm(e2) {
  ...
  if (t2 === `preparing`) return { tone: `info`, title: `Автоподбор обхода`, detail: `Сбрасываю прежнюю службу и owned WinWS перед проверкой` };
  if (t2 === `profile-start` && n2) return { tone: `info`, title: `Автоподбор обхода`, detail: `Проверяет профиль ${n2}${a2}` };
  if (t2 === `profile-result` && n2) {
    let t3 = String(e2?.result ?? ``) === `success`;
    return { tone: t3 ? `good` : `warn`, title: `Автоподбор обхода`, detail: `${n2}: ${t3 ? `успех` : `ошибка`}${o2 == null ? `` : `, ${o2} мс`}${a2}` };
  }
  if (t2 === `complete`) {
    let t3 = Q(e2?.bestProfile);
    return { tone: `good`, title: `Автоподбор завершен`, detail: t3 ? `Лучший профиль: ${t3}` : `Рабочий профиль не найден` };
  }
  return t2 === `cancelled` ? { tone: `warn`, title: `Автоподбор остановлен`, detail: `В истории оставлены только полностью проверенные профили` } : { tone: `info`, title: `Автоподбор обхода`, detail: i2 ? `Готовлю проверку ${i2} профилей` : `Готовлю проверку профилей` };
}
function Em(e2, t2) {
  let n2 = Number(e2);
...
function Bm(e2, t2) {
  ...
  return /fake tls auto/i.test(t2) ? `${r2} / FAKE TLS AUTO` : /simple fake/i.test(t2) ? `${r2} / SIMPLE FAKE` : /alt/i.test(t2) ? `${r2} / alternative WinWS profile` : `${r2} / Flowseal 5.4.0 baseline`;
}
function Vm(e2) {
