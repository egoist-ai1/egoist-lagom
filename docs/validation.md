...
| Быстрый изолированный контур | 8 этапов за 49,83 с; состояние хоста до и после совпало |
...
Скрипт `scripts/run-fast-validation.ps1` создаёт отдельный временный каталог, сохраняет снимки состояния до и после, запускает сборку, 335 тестов, Core self-test, проверку подписанных метаданных и четыре UI-набора. Первый полный прогон занял 49,83 секунды; восемь этапов завершились с кодом 0, а SHA-256 состояния хоста до и после совпал.
